...
看起來的解法：**只有真的離開選角（進入演出）才中止；選角中換角色只是「不再開新的」。**

**⚠️ 但這個改法（v1.90）在睿哥的 iPhone 上讓卡片完全不翻面，已於 v1.91 整個退掉。**
12 秒錄影、跨兩個角色一路停在卡背，連頭像後備都沒出現。
**而雲端完全重現不出來**：限速 130KB/s＋206 伺服器＋模擬他的節奏連點 8 位，
壞版本一樣全部翻牌、同時在飛的請求峰值只有 1；連「預抓 14 支全跑完」的狀態也正常。
我先懷疑是拿掉 abort 後請求疊起來吃光 Safari 的連線數 —— **寫測試去驗，假設被推翻。**

真正的教訓有兩條：
1. **這個改法會讓預抓在他的線上幾乎不間斷地跑**，而「持續佔線」正是雲端測不出來的。
2. **v1.90 一次上了四件事**（不 abort／`warmLineFree` 閘門／延後 confirm／面板 A-B 標記），
   炸了就無從二分。**重做時一次只放一個變數，而且每次都要他帶 `?debug=1` 回報。**
...
- **選角待機片第一次點會等約 1 秒 —— 仍未解決。** 原因已經量到（預抓被自己 `abort()`，
  見地雷區 ⑤），但照那個方向改的 v1.90 在他手機上讓卡片完全不翻面，已退回。
  **下一步不是想新解法，是先讓他帶 `?debug=1` 重現一次壞掉的狀態**，
  拿到 A／B 標記的事件序列再動手。
