...
`playOnce()` 舊版一設好新來源就把另一顆 `pause()` **並拿掉 `is-active`**，
可是新片還要等 `loadeddata` 才播得了 —— 中間舞台整個是空的。
睿哥 2026-09-12 的錄影量到**約 670ms**（他：「可以不要黑頻嗎」）。

規則：**收舊的要排在「新的真的要 `play()`」那一刻，不能排在「開始載入」。**
「iOS 同時只能播一支」限制的是**播放**，`play()` 前停掉就滿足；
而 `activateVideo()` 會在同一刻換 `is-active`，所以不會有空檔。

這跟地雷區 ⑱ 的循環交接是同一條原則：**準備中 ≠ 已完成**，
畫面上的東西要撐到接手者真的能播為止。

量法：按下按鈕後每 30ms 取樣「舞台上有沒有可見且 `readyState>=2` 的影片」，
印成 `■`／`□` 的時間軸。修好前 `□□□□□□□□□□□□□□□□■■■…`，修好後全 `■`。

### ㉕ 換素材時要逐片型清點，別漏掉

v1.102／v1.103 把 final 與 attack 還原到 CRF 29，**victory 被漏掉**，
一直停在 CRF 32，直到睿哥 2026-09-12 指出勝利動畫還是糊。

五種角色片型 `wait / confirm / attack / final / victory` ＋三支開場片
`boss/arrival、order/intro、teams/intro`，**動畫質時一次列完再逐項確認**。

同時要問第二個問題：**這個片型有沒有被預抓？** 勝利片當時
`prefetchFinalClip()` 只抓了它的 **poster**，影片本身完全沒抓，
兩個播放點（`victoryFilm.js`、結果頁立繪循環）都是直接跟伺服器要。
**檔案變大之前先確認預抓接得住**（地雷區 ⑳）。

### ㉖ Grid 的 `1fr` 遇到「其中一張內容比較多」就會裁掉它

`#screen-mode .mode-cards` 用 `grid-auto-rows: 1fr` 讓三張模式卡等高，
但**命運分隊那張多一排隊數按鈕**。螢幕一矮，它分到的高度裝不下自己的內容，
而卡片是 `align-content: center` ＋ `overflow: hidden` ——
內容上下各溢出一半，**上緣的 kicker 被切掉**（睿哥 2026-09-12）。
