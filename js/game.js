  ...
  async function presentRun() {
    ...
    try {
      ...
      if (result.mode === "boss" || result.mode === "doom") {
        ...
        /**
         * 開場影片（睿哥 2026-08-22 換的第二版：法師施法把一張張卡牌排成方陣 ——
         * 比第一版的金幣更貼「分隊」）。原片 1280×720／15.2Mb/s／11.5MB／6.04s。
         *
         * ⚠️ **這支是橫式的（16:9），所以要 `fit: "contain"`。**
         * 舞台是直向全螢幕、預設 `object-fit: cover`；16:9 塞進 393×660 的直式框，
         * cover 會**只留中間約三成的寬度** —— 左邊的法師與右邊的卡陣剛好各被切掉一半，
         * 整個構圖就毀了。這支的重點橫跨整個畫面寬度，裁不得，只能整張縮進來。
         * `fit` 這條路正是 v1.46 拿掉橫式魔王後**刻意留著**的後路（見 BOSS_ARRIVALS
         * 上方的註解「下次再收到橫式素材就不用重寫」），這次就是那個下次。
         * 上下會有黑邊，但舞台本來就是暗的，比切掉法師好得多。
         *
         * **不要改成裁成直式** —— 試過就知道，怎麼裁都得犧牲法師或卡陣其中一邊。
         *
         * 壓成 800×450 CRF 30 共 555KB（顯示寬度 393pt＝786 實體像素，800 剛好不必放大）。
         * `-map 0:v:0` 不能省：這支還夾了一軌 mjpeg 封面圖，不指定會被一起帶進去。
         * 音軌拿掉 —— 舞台影片一律靜音播放（iOS 自動播放的硬性條件）；
         * **但這支的音軌是有東西的（-20.1 LUFS）**，睿哥想要的話可以像抽牌那首一樣
         * 抽出來當專屬 BGM，那要另外做。
         *
