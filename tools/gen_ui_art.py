...
def knockout(path: pathlib.Path, lo: float, hi: float = 0.42) -> Image.Image:
    ...
    rgb = np.clip(src / np.maximum(alpha, 1e-3)[..., None], 0, 1)  # 反預乘
    img = Image.fromarray(
...
def main() -> None:
    ...
    full = knockout(pathlib.Path(args.crest), lo=0.06).resize((512, 512), Image.LANCZOS)
